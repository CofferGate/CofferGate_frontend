...
- PR 본문은 변경 이유, 변경 내용, 검증 결과, 영향 및 위험 순서로 작성합니다.
- 변경 이유에는 이 작업이 필요한 배경과 달성하려는 목적을 기록합니다.
- 변경 내용에는 리뷰어가 확인해야 할 구현·설정·계약 변경을 구체적으로 기록합니다.
- 영향 및 위험에는 호환성, 보안 경계, 후속 작업, 아직 연결되지 않은 외부 의존성을 기록합니다.
